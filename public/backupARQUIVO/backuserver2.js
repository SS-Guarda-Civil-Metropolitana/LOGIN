...
app.post('/cadastrar', async (req, res) => {
    ...
    try {
        ...
        const checkQuery = `
            ...
            WHERE cpf = ? OR email = ? OR matricula = ? OR nome_guerra = ? LIMIT 1
        `;
...
app.post('/api/login', async (req, res) => {
    ...
    try {
        ...
        // --- Logs de Depuração ---
        console.log('LOGIN: Usuário encontrado no DB:', user.nome_completo, user.nome_guerra, user.email);
        ...
        console.log('LOGIN: Tipo do hash do DB (antes da conversão):', typeof user.senha);
        // --- Fim dos Logs ---
        ...
        console.log('LOGIN: Resultado da comparação (isMatch):', isMatch); // LOG CRÍTICO
...
app.post('/api/moradores', async (req, res) => {
    try {
        ...
        const {
            nomeCompleto, vulgo, antecedentes, vicios, nomeMae,
            cpf, rg, endereco, naturalidade, informacao,
